...
async function ensureTable() {
  ...
  _init = (async () => {
    ...
    // quando o lead entrou no status atual (ordena os cards do Kanban pela data em que caíram na coluna).
    // Nas linhas antigas cai pro created_at — não há histórico de mudanças anteriores a esta migração.
    await sql`ALTER TABLE leads ADD COLUMN IF NOT EXISTS status_at timestamptz`;
    await sql`UPDATE leads SET status_at = created_at WHERE status_at IS NULL`;
    await sql`ALTER TABLE leads ALTER COLUMN status_at SET DEFAULT now()`;
    // chave de idempotência: a mesma sessão do quiz reenviando (toque duplo, Enter+clique, voltar e corrigir o número)
    // ATUALIZA a mesma linha em vez de criar um lead duplicado. NULL não conflita, então leads antigos e o webinar seguem iguais.
    await sql`ALTER TABLE leads ADD COLUMN IF NOT EXISTS session_id text`;
    await sql`CREATE UNIQUE INDEX IF NOT EXISTS leads_session_id_uq ON leads(session_id)`;
